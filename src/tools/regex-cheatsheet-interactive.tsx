...
import {
  ...
  filterCheatsheet,
} from "@/lib/regex/cheatsheet";
import { track } from "@/lib/analytics";

export function RegexCheatsheetInteractiveTool() {
  ...
  const markStart = useCallback(() => {
    if (!started) {
      ...
      track({
        name: "tool_start",
        tool: "regex-cheatsheet-interactive",
        family: "tools",
      });
    }
  }, [started]);

  const filtered = useMemo(() => filterCheatsheet(query), [query]);
  const visible =
    activeId === "all"
      ? filtered
      : filtered.filter((c) => c.id === activeId);

  const total = countCheatsheetEntries(
    activeId === "all"
      ? filtered
      : filtered.filter((c) => c.id === activeId),
  );
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <p className="text-sm text-[var(--muted)]">
          {total} entries · JavaScript (ECMAScript) flavor
        </p>
        <Link
          ...
          <section key={cat.id} className="space-y-3">
            ...
            <ul className="grid gap-2 sm:grid-cols-2">
              {cat.entries.map((entry) => (
                <li key={`${cat.id}-${entry.token}-${entry.name}`}>
                  <button
                    type="button"
                    onClick={() => copyToken(entry.token)}
                    className="flex h-full w-full flex-col rounded-xl border border-[var(--border)] bg-[var(--surface)] px-4 py-3 text-left transition hover:border-[var(--copper)]/45"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <code className="font-[family-name:var(--font-mono)] text-sm text-[var(--copper-bright)]">
                        {entry.token}
                      </code>
                      <span className="text-xs text-[var(--muted)]">
                        {copied === entry.token ? "Copied" : "Copy"}
                      </span>
                    </div>
