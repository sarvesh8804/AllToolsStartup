/** Pure MD5 implementation (RFC 1321). Returns lowercase hex. */
...
export function md5Bytes(bytes: Uint8Array): string {
  ...
  const lenIndex = (((bitLen + 64) >>> 9) << 4) + 14;
  while (words.length <= lenIndex + 1) words.push(0);
...
export function md5(message: string): string {
  return md5Bytes(new TextEncoder().encode(message));
}
